...
def listeMortAppend(listeMort, stats):
    listeMort.append((time.time() - stats['creer'])*1000)
    return listeMort
...
for i in range(nbGen):
    ...
    dureeListe.append(sum(listeMort) / len(listeMort))

    population = creerPopulation()
    print(vic, '  ',barreProgression(i+1, nbGen), end='\r')
print('\n')

plt.title('Précision et Vue')
plt.xlabel('Génération')
plt.ylabel('Durée moyenne (ms)')

m, b = np.polyfit(l, dureeListe, 1)
aj_lineaire = np.poly1d([m, b])
plt.plot(l, aj_lineaire(l), label='Duree', color='#33FF7A')
plt.plot(l, dureeListe, label='Duree Point', color='#FF3B33', alpha=0.3)

plt.yticks(list(range(0, 10**4, 500)))
